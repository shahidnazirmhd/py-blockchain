    ...
    def save_data(self):
        try:
            with open("../data/blockchain_data.txt", mode="w") as f:
                dict_blockchain = [block.__dict__ for block in [Block(block_el.index, block_el.previous_hash, [tx.to_ordered_dict() for tx in block_el.transactions], block_el.proof, block_el.timestamp) for block_el in self.chain]]
                dict_open_transactions = [tx.__dict__ for tx in self.open_transactions]
